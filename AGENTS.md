...
- **SFTP virtual FS**: `SftpService` + `SftpFileSystemProvider` (`maixsftp://`). Separate SSH connection from shell (`SftpSession`). Command `maixcode.openDeviceSftp` adds a workspace folder (`MaixSFTP: <name>`) and reveals it in Explorer. URI: `maixsftp://<authority>/<remote/abs/path>` (authority = sanitized device name or IP). Config: `maixcode.sftpRoot` (default `/`), `maixcode.sftpReadOnly`, `maixcode.sftpHidePatterns` (glob `*`/`?`/`**` or `/regex/flags`; applied in `readDirectory` on basename + full path). Shared credential helper: `src/service/ssh/credentials.ts`. UI: Current Device Info + inline on `maixcode-deviceIp` (`inline@3`). Provider supports stat/read/write/mkdir/delete/rename/copy; watch is no-op.
