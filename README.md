...
1. 打开右侧栏 → **MaixCode** → **Image**，或命令 `MaixCode: Open Image Viewer`
...
- 凭据通过 `maixcode.sshCredentials` 配置，依次尝试直到成功（默认 `root / root`）
...
| `maixcode.imageViewerAutoStart` | `true` | 连接设备后自动开始推流 |
| `maixcode.sshPort` | `22` | SSH 端口 |
| `maixcode.sshConnectTimeoutMs` | `10000` | SSH 连接超时 |
| `maixcode.sshCredentials` | `root/root` | SSH 凭据列表（按序尝试） |
...
pnpm run watch        # webpack watch（F5 调试用）
pnpm run lint         # ESLint
pnpm run test         # 运行测试
pnpm run package:vsix # 打包 VSIX
```
